...
function Modify-MachineCodeConfig {
    ...
    if (-not (Test-Path $configPath)) {
        ...
        Write-Host "$BLUE  1  Manually start Cursor application$NC"
        Write-Host "$BLUE  2  Wait for Cursor to fully load (about 30 seconds)$NC"
        Write-Host "$BLUE  3  Close Cursor application$NC"
        Write-Host "$BLUE  4  Re-run this script$NC"
        Write-Host ""
...
Write-Host "$YELLOW📱  Follow Official Account [Xone AI] $NC"
...
Write-Host "$YELLOW💡  [Important] This tool is free. If it helps you, please follow the official account [Xone AI]  $NC"
Write-Host ""
Write-Host "$YELLOW⚡  [Advertisement] Official Cursor Accounts: Pro¥65 | Pro+¥265 | Ultra¥888 Exclusive account/7-day warranty, Contact: +84587646666 (Zalo)  $NC"
Write-Host "$BLUE================================$NC"
...
if ($executeMode -eq "MODIFY_ONLY") {
    ...
    Write-Host "$BLUE  1  Detect Cursor configuration file$NC"
    Write-Host "$BLUE  2  Backup existing configuration file$NC"
    Write-Host "$BLUE  3  Modify machine code configuration$NC"
    Write-Host "$BLUE  4  Display operation completion information$NC"
    Write-Host ""
    ...
    Write-Host "$BLUE  1  Detect and close Cursor processes$NC"
    Write-Host "$BLUE  2  Save Cursor program path information$NC"
    ...
    Write-Host "$BLUE  4  Restart Cursor to generate new configuration file$NC"
    Write-Host "$BLUE  5  Wait for configuration file generation (up to 45 seconds)$NC"
    Write-Host "$BLUE  6  Close Cursor process$NC"
    Write-Host "$BLUE  7  Modify newly generated machine code configuration file$NC"
...
if ($executeMode -eq "MODIFY_ONLY") {
    ...
    if (-not $envCheck.Success) {
        ...
        Write-Host "$BLUE  2  Manually start Cursor once, then re-run the script$NC"
...
Write-Host ""
Write-Host "$GREEN================================$NC"
Write-Host "$YELLOW📱  Follow Official Account [XoneVN AI] to share more Cursor tips and AI knowledge (Script is free, follow the account to join group for more tips and experts)  $NC"
Write-Host "$YELLOW⚡   [Advertisement] Official Cursor Accounts: Pro¥65 | Pro+¥265 | Ultra¥888 Exclusive account/7-day warranty, Contact: +84587646666 (Zalo)  $NC"
Write-Host "$GREEN================================$NC"
Write-Host ""
